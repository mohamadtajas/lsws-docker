@can('trendyol_manual_order')
    <div class="row">
        <div class="col-10 mx-auto my-5">
            <div class="text-left">
                <form action="{{ route('trendyol_update_order_number') }}" method="POST" autocomplete="off">
                    @csrf
                    <input type="hidden" name="orderId" value="{{ $order->id }}">
                    <div class="form-group row" id="brand">
                        <label class="col-12 col-from-label fs-13">{{ translate('Trendyol Order Number') }}</label>
                        <label class="col-12 col-from-label fs-13">{{ translate('Num. of Items') }} : {{ $order->orderDetails->pluck('quantity')->sum() }}</label>
                        <div class="col-12">
