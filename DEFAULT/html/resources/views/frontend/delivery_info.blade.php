                ...
                <div class="col-xxl-8 col-xl-10 mx-auto">
                    <div class="border bg-white p-4 mb-4">
                        <form class="form-default" action="{{ route('checkout.store_delivery_info') }}" role="form"
                                ...
                                foreach ($carts as $key => $cartItem) {
                                    if ($cartItem['trendyol'] == 0 && $cartItem['provider_id'] == null) {
                                        ...
                                        array_push($trendyol_products, [
                                            ...
                                            'urunNo' => $cartItem['urunNo'],
                                            'variation' => $cartItem['variation'],
                                        ]);
                                    } elseif ($cartItem['provider_id'] != null) {
                                        $providers_products_id[$cartItem['provider_id']][$cartItem['id']] =
                                            $cartItem['product_id'];
                                    }
                                }
                                foreach ($providers_products_id as $key => $provider_products_id) {
                                    $provider = \App\Models\Provider::find($key);
                                    if ($provider) {
                                        array_push($providers_products, ...$provider
                                            ->Service()
                                            ->productsDetails($provider_products_id));
                                    }
                                }
                                ...
                                @foreach ($trendyol_products as $trendyol_product)
                                    @php
                                        ...
                                        $product = trendyol_product_details(
                                            $accaessToken,
                                            $trendyol_product['id'],
                                            $trendyol_product['urunNo'],
                                        );
                                    @endphp
                                    <div class="card mb-5 border-0 rounded-0 shadow-none">
                                        <div class="card-header py-3 px-0 border-bottom-0">
                                            <h5 class="fs-16 fw-700 text-dark mb-0">{{ $product['shopName'] }}
                                                {{ translate('Products') }}</h5>
                                        </div>
                                        <div class="card-body p-0">
                                            <!-- Product List -->
                                                    ...
                                                    <div class="d-flex align-items-center">
                                                        <span class="mr-2 mr-md-3">
                                                            <img src="{{ $product['photos'][0] }}"
                                                                class="img-fit size-60px" alt="{{ $product['name'] }}"
                                                                onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder.webp') }}';">
                                                        </span>
                                                        <span class="fs-14 fw-400 text-dark">
                                                            {{ $product['name'] }}
                                                            <br>
                                                            @if ($trendyol_product['variation'] != '')
                                                                <span
                                                                    class="fs-12 text-secondary">{{ translate('Variation') }}:
                                                                    {{ $trendyol_product['variation'] }}</span>
                                                            @endif
                                                        </span>
                                                    </div>
                                                </li>
                                                ...
                                                <div class="row pt-3">
                                                    ...
                                                    <div class="col-md-6">
                                                        <div class="row gutters-5">
                                                            ...
                                                            @if (get_setting('shipping_type') != 'carrier_wise_shipping')
                                                                <div class="col-6">
                                                                    <label class="aiz-megabox d-block bg-white mb-0">
                                                                        <input type="radio"
                                                                            name="shipping_type_{{ $product['id'] }}{{ get_admin()->id }}"
                                                                            value="home_delivery"
                                                                            onchange="show_pickup_point(this, 'admin')"
                                                                            data-target=".pickup_point_id_admin" checked>
                                                                        <span class="d-flex aiz-megabox-elem rounded-0"
                                                        ...
                                                        @if ($pickup_point_list)
                                                            <div class="mt-3 pickup_point_id_admin d-none">
                                                                <select class="form-control aiz-selectpicker rounded-0"
                                                                    name="pickup_point_id_{{ $product['id'] }}{{ get_admin()->id }}"
                                                                    data-live-search="true">
                                                                    <option>
                                                                        {{ translate('Select your nearest pickup point') }}
                                                                    </option>
                            ...
                            <!-- Provider Products -->
                            @if (!empty($providers_products))
                                @foreach ($providers_products as $key => $provider_product)
                                    <div class="card mb-5 border-0 rounded-0 shadow-none">
                                        <div class="card-header py-3 px-0 border-bottom-0">
                                            <h5 class="fs-16 fw-700 text-dark mb-0">{{ $provider_product['shopName'] ?? '' }}
                                                {{ translate('Products') }}</h5>
                                        </div>
                                        <div class="card-body p-0">
                                            <!-- Product List -->
                                            <ul class="list-group list-group-flush border p-3 mb-3">
                                                ...
                                                <li class="list-group-item">
                                                    <div class="d-flex align-items-center">
                                                        <span class="mr-2 mr-md-3">
                                                            <img src="{{ $provider_product['thumbnail'] }}"
                                                                class="img-fit size-60px" alt="{{ $provider_product['name'] }}"
                                                                onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder.webp') }}';">
                                                        </span>
                                                        <span class="fs-14 fw-400 text-dark">
                                                            {{ $provider_product['name'] }}
                                                            <br>
                                                            @if (isset($provider_product['variation']) && $provider_product['variation'] != '')
                                                                <span
                                                                    class="fs-12 text-secondary">{{ translate('Variation') }}:
                                                                    {{ $provider_product['variation'] }}</span>
                                                            @endif
                                                        </span>
                                                    </div>
                                                </li>
                                                ...
                                                <div class="row pt-3">
                                                    ...
                                                    <div class="col-md-6">
                                                        <div class="row gutters-5">
                                                            ...
                                                            @if (get_setting('shipping_type') != 'carrier_wise_shipping')
                                                                <div class="col-6">
                                                                    <label class="aiz-megabox d-block bg-white mb-0">
                                                                        <input type="radio"
                                                                            name="shipping_type_{{ $provider_product['id'] }}{{ get_admin()->id }}"
                                                                            value="home_delivery"
                                                                            onchange="show_pickup_point(this, 'admin')"
                                                                            data-target=".pickup_point_id_admin" checked>
                                                                        <span class="d-flex aiz-megabox-elem rounded-0"
                                                        ...
                                                        @if ($pickup_point_list)
                                                            <div class="mt-3 pickup_point_id_admin d-none">
                                                                <select class="form-control aiz-selectpicker rounded-0"
                                                                    name="pickup_point_id_{{ $provider_product['id'] }}{{ get_admin()->id }}"
                                                                    data-live-search="true">
                                                                    <option>
                                                                        {{ translate('Select your nearest pickup point') }}
                                                                    </option>
