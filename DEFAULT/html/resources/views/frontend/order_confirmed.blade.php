                                        ...
                                        <table class="table table-responsive-md text-soft-dark fs-14">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="opacity-60 border-top-0">{{ translate('Price') }}</th>
                                                    <th class="text-right opacity-60 border-top-0 pr-0">
                                                        {{ translate('Tracking') }}</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                @foreach ($order->orderDetails as $key => $orderDetail)
                                                    <tr>
                                                        ...
                                                        <td class="border-top-0 border-bottom">
                                                            @if ($orderDetail->product != null || $orderDetail->trendyol == 1 || $orderDetail->provider_id != null)
                                                                ...
                                                                @elseif($orderDetail->trendyol == 1)
                                                                    @php
                                                                        $product = trendyol_product_details(
                                                                            trendyol_account_login(),
                                                                            $orderDetail->product_id,
                                                                            $orderDetail->urunNo,
                                                                        );
                                                                    @endphp
                                                                    <a href="{{ route('trendyol-product', ['id' => $product['id'], 'urunNo' => $product['urunNo']]) }}"
                                                                        target="_blank" class="text-reset">
                                                                        {{ $product['name'] }}
                                                                    </a>
                                                                @elseif($orderDetail->provider_id != null)
                                                                    @php
                                                                        $product = $orderDetail->provider->service()->productDetails($orderDetail->product_id);
                                                                    @endphp
