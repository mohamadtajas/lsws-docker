...
@section('content')
                                    ...
                                    <a href="{{ json_decode(get_setting('home_slider_links'), true)[$key] }}">
                                        ...
                                        <div
                                            ...
                                            <img class="img-fit h-100 m-auto has-transition ls-is-cached lazyloaded"
                                                src="{{ $slider ? my_asset($slider->file_name) : static_asset('assets/img/placeholder.webp') }}"
                                                alt="{{ env('APP_NAME') }} promo"
                                                onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder-rect.webp') }}';">
                                        </div>
                                    </a>
                                </div>
                            @endforeach
    ...
    @if (count($featured_categories) > 0)
        <section class="mb-2 mb-md-2 mt-2 mt-md-0">
            <div class="container">
                <!-- Categories -->
                <div class="bg-white px-sm-3">
                    <div class="aiz-carousel sm-gutters-17" data-items="6" data-xxl-items="6" data-xl-items="6"
                        data-lg-items="6" data-md-items="5" data-sm-items="4" data-xs-items="3" data-arrows="false"
                        ...
                        @foreach ($featured_categories as $key => $category)
                            ...
                            <div class="carousel-box position-relative p-0 has-transition">
                                <div class="h-100px h-sm-100px h-md-100px h-lg-150px h-xxl-200px w-100px w-sm-100px w-md-100px w-lg-150px w-xxl-200px mx-auto"
                                    style="box-shadow:5px 5px 9px 2px #83838357; border-radius: 25px;margin-top: 15px;margin-bottom: 15px;">
                                    <div class="h-100 w-100 w-xl-auto position-relative hov-scale-img overflow-hidden">
                                        <div class="position-absolute h-100 w-100 overflow-hidden"
                                            ...
                                            <a href="{{ route('products.category', $category->slug) }}">
                                                <img src="{{ !empty($category->cover_image) ? uploaded_asset($category->cover_image) : static_asset('assets/img/placeholder.webp') }}"
                                                    alt="{{ $category_name }}" style="border-radius : 20px "
        ...
        <section class="mb-2 mb-md-3 mt-2 mt-md-3"
            ...
            <div class="container">
                ...
                <div class="row no-gutters align-items-center"
                    style="background: {{ $flash_deal_bg }}; border-radius:20px">
                    <!-- Flash Deals Baner & Countdown -->
                    <div class="col-xxl-4 col-lg-5 col-6 h-200px h-md-400px h-lg-475px">
                        <div class="h-100 w-100 w-xl-auto"
                            style="background-image: url('{{ uploaded_asset($flash_deal->banner) }}'); background-size: cover; background-position: center center; border-radius:20px">
                            <div class="py-5 px-md-3 px-xl-5 d-none d-md-block">
                                <div class="bg-white" style="border-radius: 20px">
                                    <div class="aiz-count-down-circle" style="border-radius: 20px"
    ...
    <div id="suggestions_products">

    </div>

    <!-- Banner Section 3 -->
    @if (get_setting('home_banner3_images') != null)
        <div class="mb-2 mb-md-2 mt-2 mt-md-0">
            <div class="container" id="banner-section-3-container">
                @php
                    $banner_3_imags = array_slice(json_decode(get_setting('home_banner3_images', null, $lang)), 0, 24);
                    foreach ($banner_3_imags as $key => $value) {
                        $banner_3_imags[$key] = uploaded_asset($value);
                    }
                ...
                @foreach ($banner_3_imags as $key => $value)
                    @if ($count % 3 == 0)
                        <div class="aiz-carousel gutters-16 overflow-hidden arrow-inactive-none arrow-dark arrow-x-15"
                            data-items="3" data-xxl-items="3" data-xl-items="3" data-lg-items="{{ $data_md }}"
                            data-md-items="{{ $data_md }}" data-sm-items="1" data-xs-items="1" data-arrows="true"
                            data-dots="false">
                    @endif
                    <div class="carousel-box overflow-hidden hov-scale-img px-0">
                        <a @if (json_decode(get_setting('home_banner3_links'), true) &&
                                isset(json_decode(get_setting('home_banner3_links'), true)[$key])) href=" {{ json_decode(get_setting('home_banner3_links'), true)[$key] }}" @endif
                            class="d-block text-reset overflow-hidden">
                            <img src="{{ static_asset('assets/img/placeholder-rect.webp') }}" loading="eager|lazy"
                                data-src="{{ $value }}" alt="{{ env('APP_NAME') }} promo"
                                class="img-fluid lazyload w-100 has-transition"
                                onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder-rect.webp') }}';"
                                style="padding :3%!important; border-radius : 25px !important;">
                        </a>
                    </div>
                    @php
                        $count++;
                    @endphp
                    @if ($count % 3 == 0 || $count == count($banner_3_imags))
            </div>
    @endif
    @endforeach
    </div>
                        ...
                        @foreach ($banner_1_imags as $key => $value)
                            <div class="carousel-box overflow-hidden hov-scale-img">
                                <a href="{{ json_decode(get_setting('home_banner1_links'), true)[$key] }}"
                                    class="d-block text-reset overflow-hidden">
                                    <img src="{{ static_asset('assets/img/placeholder-rect.webp') }}"
                                            ...
                                            <div class="col-4 col-md-5 mb-3 mb-md-0">
                                                <a href="{{ route('customer.product', $classified_product->slug) }}"
                                                    ...
                                                    <img class="img-fluid lazyload mx-auto has-transition"
                                                        ...
                                                        data-src="{{ isset($classified_product->thumbnail->file_name) ? my_asset($classified_product->thumbnail->file_name) : static_asset('assets/img/placeholder.webp') }}"
                        ...
                        @foreach ($brands as $brand)
                            <div
                                ...
                                <a href="{{ route('products.brand', $brand->slug) }}" class="d-block p-sm-3">
                                    <img src="{{ isset($brand->brandLogo->file_name) ? my_asset($brand->brandLogo->file_name) : static_asset('assets/img/placeholder.webp') }}"
...
@push('script')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function loadBannerImages(page) {
                if (loading) return;
                loading = true;

                fetch(`{{ route('home.banner.section3') }}?page=${page}`)
                    .then(response => response.json())
                    .then(data => {
                        if (data.images && data.images.length > 0) {
                            const container = document.getElementById('banner-section-3-container');
                            let carouselHtml = '';

                            data.images.forEach((imageSrc, index) => {
                                const imageLink = data.links && data.links[index] ? data.links[index] :
                                    '#';

                                // Start a new carousel group every 3 images
                                if (index % 3 === 0) {
                                    if (index !== 0) {
                                        carouselHtml += '</div>';
                                    }
                                    carouselHtml +=
                                        '<div class="aiz-carousel gutters-16 overflow-hidden arrow-inactive-none arrow-dark arrow-x-15 new-carousel" ' +
                                        'data-items="3" data-xxl-items="3" data-xl-items="3" data-lg-items="2" data-md-items="2" data-sm-items="1" data-xs-items="1" data-arrows="true" data-dots="false">';
                                }

                                carouselHtml += `
                                <div class="carousel-box overflow-hidden hov-scale-img px-0">
                                    <a href="${imageLink}" class="d-block text-reset overflow-hidden">
                                        <img src="{{ static_asset('assets/img/placeholder-rect.webp') }}"
                                            data-src="${imageSrc}" alt="{{ env('APP_NAME') }} promo"
                                            class="img-fluid lazyload w-100 has-transition"
                                            onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder-rect.webp') }}';"
                                            style="padding: 3%!important; border-radius: 25px !important;">
                                    </a>
                            ...
                            // Initialize only the newly added carousels
                            $(".new-carousel").each(function() {
                                initAizCarousel(this);
                            }).removeClass('new-carousel');
            ...
            window.addEventListener('scroll', () => {
                let scrollThreshold = 2700;

                if (window.innerWidth <= 768) {
                    scrollThreshold = 2500;
                }

                if (nextPage && (window.innerHeight + window.scrollY) >= document.body.offsetHeight -
                    scrollThreshold) {
                    loadBannerImages(nextPage);
                }
            });
        });

        function initAizCarousel(element) {
            var a = $(element),
                t = a.data("xs-items") || 1,
                l = a.data("sm-items") || t,
                i = a.data("md-items") || l,
                ...
                u = a.data("rows") || 1,
                p = !!a.data("autoplay"),
                f = a.data("autoplay-speed") || 1500,
                m = !!a.data("fade"),
                h = a.data("nav-for") || null,
                v = !!a.data("infinite"),
            ...
            // RTL Support
            var _ = $("html").attr("dir") === "rtl" && !y,
                F = $("html").attr("dir") === "rtl" && !C,
                S = $("html").attr("dir") === "rtl" && !z,
                U = $("html").attr("dir") === "rtl" && !w,
                T = $("html").attr("dir") === "rtl" && !x,
                D = $("html").attr("dir") === "rtl" && !k;

            // Initialize Slick carousel with all options from data attributes
            a.slick({
                slidesToShow: s,
                slidesToScroll: 1,
                autoplay: p,
