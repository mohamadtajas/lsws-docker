    ...
    <div class="position-relative h-140px h-md-200px img-fit overflow-hidden">
        ...
        @if ($product->auction_product == 0)
            ...
            <div class="absolute-top-right aiz-p-hov-icon">
                <a href="javascript:void(0)" class="hov-svg-white" onclick="addToWishList({{ $product->id }} , '' , 0)"
                ...
                <a href="javascript:void(0)" class="hov-svg-white" onclick="addToCompare({{ $product->id }} , '' , 0)"
            ...
            <a class="cart-btn absolute-bottom-left w-100 h-35px aiz-p-hov-icon text-white fs-13 fw-700 d-flex flex-column justify-content-center align-items-center @if (in_array($product->id, $cart_added)) active @endif"
                ...
                @if (Auth::check()) onclick="showAddToCartModal({{ $product->id }} , 0)" @else onclick="showLoginModal()" @endif>
    ...
    <div class="p-2 p-md-3 text-left">
        ...
        <h3 class="fw-400 fs-13 text-truncate-2 lh-1-4 mb-0 h-35px text-center">
            <a href="{{ $product_url }}" class="d-block text-reset hov-text-primary"
                title="{{ $product->getTranslation('name') }}"><b class="notranslate">{{ $product->brand->name ?? ''}} - </b>{{ $product->getTranslation('name') }}</a>
        </h3>
