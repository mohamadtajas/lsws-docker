...
<div class="aiz-card-box h-auto bg-white py-3 hov-scale-img">
    <div class="position-relative h-140px h-md-200px img-fit overflow-hidden">
        @php
            $product_url = route('trendyol-product', ['id' => $product['id'], 'urunNo' => $product['urunNo']]);
            if ($product['auction_product']== 1) {
                $product_url = route('auction-product', $product['slug']);
            }
        @endphp
        <!-- Image -->
        <a href="{{ $product_url }}" class="d-block h-100" target="_blank">
            <img class="lazyload mx-auto img-fit has-transition"
                src="{{ $product['thumbnail'] }}"
                alt="{{ $product['resimAlt'] }}" title="{{ $product['resimAlt'] }}"
        ...
        @if ($product['discount_price'] > 0)
            <span class="absolute-top-left bg-primary ml-1 mt-1 fs-11 fw-700 text-white w-35px text-center"
                style="padding-top:2px;padding-bottom:2px;">-{{ $product['discount_price'] }}%</span>
        @endif
        <!-- Wholesale tag -->
        @if ($product['wholesale_product'])
            <span class="absolute-top-left fs-11 text-white fw-700 px-2 lh-1-8 ml-1 mt-1"
                style="background-color: #455a64; @if ($product['wholesale_product'] > 0) top:25px; @endif">
                {{ translate('Wholesale') }}
            </span>
        @endif
        @if ($product['auction_product'] == 0)
            ...
            <div class="absolute-top-right aiz-p-hov-icon">
                <a href="javascript:void(0)" class="hov-svg-white" onclick="addToWishList({{ $product['id'] }} , '{{ $product['urunNo'] }}' , 1)"
                ...
                <a href="javascript:void(0)" class="hov-svg-white" onclick="addToCompare({{ $product['id'] }} , '{{ $product['name'] }}' , 1 , '{{ $product['urunNo'] }}')"
            ...
            <a class="cart-btn absolute-bottom-left w-100 h-35px aiz-p-hov-icon text-white fs-13 fw-700 d-flex flex-column justify-content-center align-items-center @if (in_array($product['id'], $cart_added)) active @endif"
                ...
                @if (Auth::check()) onclick="showAddToCartModal({{ $product['id'] }},1,{{ $product['urunNo'] }})" @else onclick="showLoginModal()" @endif>
        ...
        @if (
            ...
            @php
                ...
                $highest_bid = $product['unit_price'];
                $min_bid_amount = $highest_bid != null ? $highest_bid + 1 : $product['unit_price'];
            @endphp
            <a class="cart-btn absolute-bottom-left w-100 h-35px aiz-p-hov-icon text-white fs-13 fw-700 d-flex flex-column justify-content-center align-items-center @if (in_array($product['id'], $cart_added)) active @endif"
    ...
    <div class="p-2 p-md-3 text-left">
        ...
        <h3 class="fw-400 fs-13 text-truncate-2 lh-1-4 mb-0 h-35px text-center">
            <a href="{{ $product_url }}" class="d-block text-reset hov-text-primary"
                title="{{ $product['name'] }}"><b class="notranslate">{{ $product['brandName'] }} - </b> {{ $product['name'] }}</a>
        </h3>
        <div class="fs-14 d-flex justify-content-center mt-3">
            @if ($product['auction_product'] == 0)
                <!-- Previous price -->
                @if ( $product['unit_price'] !=  $product['new_price'] )
                    <div class="disc-amount has-transition notranslate">
                        <del class="fw-400 text-secondary mr-1">{{ $product['unit_price']}}</del>
                    </div>
                @endif
                <!-- price -->
