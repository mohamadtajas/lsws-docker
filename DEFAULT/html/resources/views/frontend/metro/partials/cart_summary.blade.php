<div class="card rounded-0 border shadow-none">
    ...
    <div class="card-header pt-4 pb-1 border-bottom-0">
        ...
        <div class="text-right">
                ...
                @foreach ($carts as $key => $cartItem)
                    @php
                        if ($cartItem['trendyol'] == 0) {
                            ...
                            $product = new Product($trendyolProducts[$cartItem['product_id']]);
                        }
                    @endphp
            ...
            @php
                if (Auth::check() && get_setting('invite_system') == 1 && Auth::user()->invitation->count() == 1) {
            ...
            @foreach ($carts as $key => $cartItem)
                ...
                @elseif($cartItem['trendyol'] == 1)
                    @php $subtotal_for_min_order_amount += trendyol_product_indirimliFiyat(trendyol_account_login(), $cartItem['product_id'] , $cartItem['urunNo']) * $cartItem['quantity']; @endphp
                @elseif($cartItem['provider_id'] != null)
                    @php $subtotal_for_min_order_amount += $providerProducts[$cartItem['id']]['new_price'] * $cartItem['quantity']; @endphp
                @endif
            @endforeach
                ...
                @foreach ($carts as $key => $cartItem)
                    @php
                        ...
                        if ($cartItem['trendyol'] == 0 && $cartItem['provider_id'] == null) {
                            ...
                            $product = $trendyolProducts[$cartItem['product_id']];
                            ...
                            $tax += 0;
                            $product_name_with_choice = $product['name'];
                            if (isset($cartItem['variant']) && $cartItem['variant'] != null) {
                                $product_name_with_choice = $product['name'] . ' - ' . $cartItem['variant'];
                            }
                        }
                    ...
                    <tr class="cart_item">
                        ...
                        <td class="product-total text-right pr-0 fs-14 text-primary fw-600 border-top-0 border-bottom">
                            ...
                            @elseif($cartItem['trendyol'] == 1)
                                <span
                                    class="pl-4 pr-0">{{ single_price(floatval(trendyol_product_indirimliFiyat(trendyol_account_login(), $cartItem['product_id'], $cartItem['urunNo']) * $cartItem['quantity'])) }}</span>
                            @elseif($cartItem['provider_id'] != null)
                                <span
                                    class="pl-4 pr-0">{{  single_price($providerProducts[$cartItem['id']]['new_price'] * $cartItem['quantity']) }}</span>
                            @endif
                        </td>
                    </tr>
                @endforeach
        ...
        <table class="table" style="margin-top: 2rem!important;">
            <tfoot>
                ...
                @if ($invite_discount > 0)
                    <tr class="cart-shipping">
                        <th class="pl-0 fs-14 pt-0 pb-2 text-dark fw-600 border-top-0">
                            {{ translate('Invite Discount Coupon') }}</th>
                        <td class="text-right pr-0 fs-14 pt-0 pb-2 fw-600 text-primary border-top-0">
                            <span class="fw-600">{{ single_price($invite_discount) }}</span>
                        </td>
                    </tr>
                @endif
                        ...
                        <div class="input-group">
                            ...
                            <div class="input-group-append">
                                <button type="button" id="coupon-remove"
                                    class="btn btn-primary">{{ translate('Remove Coupon') }}</button>
                            </div>
                        </div>
                    </form>
                </div>
                        ...
                        <div class="input-group">
                            ...
                            <div class="input-group-append">
                                <button type="button" id="coupon-apply"
                                    class="btn btn-success rounded-0">{{ translate('Apply') }}</button>
                            </div>
                        </div>
                    </form>
                </div>
