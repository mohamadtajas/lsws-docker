<div class="text-left">
    ...
    <div class="row align-items-center mb-3">
        ...
        @if ($detailedProduct['est_shipping_days'] > 0)
            ...
            @if ($detailedProduct['auction_product'] != 1)
                <div class="d-flex">
                    ...
                    <a href="javascript:void(0)"
                        onclick="addToWishList({{ $detailedProduct['id'] }} , '{{ $detailedProduct['urunNo'] }}' , 1)"
                    ...
                    <a href="javascript:void(0)"
                        onclick="addToCompare({{ $detailedProduct['id'] }} , '{{ $detailedProduct['name'] }}' , 1 , '{{ $detailedProduct['urunNo'] }}')"
    ...
    @if ($detailedProduct['categoryName'] != null)
        <div class="d-flex flex-wrap align-items-center mb-3">
            ...
            <a href="{{ $detailedProduct['categoryLink'] }}"
                class="text-reset hov-text-primary fs-14 fw-700">{{ $detailedProduct['categoryName'] }}</a>
        </div>
    @endif
                ...
                <div class="row no-gutters mb-3">
                    ...
                    <div class="col-sm-10">
                        <div class="d-flex align-items-center">
                            ...
                            <strong class="fs-16 fw-700 text-primary notranslate">
                                {{ single_price($detailedProduct['new_price']) }}
                            </strong>
                            <!-- Home Price -->
                            <del class="fs-14 opacity-60 ml-2 notranslate">
    ...
    @if ($detailedProduct['auction_product'] != 1)
        <form id="option-choice-form">
            ...
            <input type="hidden" name="urunNo" value="{{ $detailedProduct['urunNo'] }}">
            <input type="hidden" name="trendyol" value="1">
                                        ...
                                        @foreach ($choice['values'] as $key => $value)
                                            <label class="aiz-megabox pl-0 mr-2 mb-0">
                                                <input type="radio" name="attribute_id_{{ $choice['id'] }}"
                                                    value="{{ $key }}"
                                                    @if ($detailedProduct['varyantlar'] == $key) checked @endif
                                                    onchange="window.location.href = `{{ route('trendyol-product', ['id' => $detailedProduct['id'], 'urunNo' => $value]) }}`">
                                                <span
                                            ...
                                            <div class="carousel-box overflow-hidden hov-scale-img px-0">
                                                <label class="aiz-megabox pl-0 mr-2 mb-0">
                                                    <input type="radio" class="not"
                                                        @if ($detailedProduct['id'] == $key) checked @endif
                                                        onchange="window.location.href = `{{ route('trendyol-product', ['id' => $key, 'urunNo' => null]) }}`">
            ...
            <div class="row no-gutters pb-3" id="chosen_price_div">
                ...
                <div class="col-sm-10">
                    <div class="product-price">
                        <strong id="chosen_price" class="fs-20 fw-700 text-primary notranslate">
                            {{ single_price($detailedProduct['new_price']) }}
                        </strong>
                    </div>
                </div>
