...
@section('panel_content')
    ...
    <div class="card rounded-0 shadow-none border">
        <div class="card-header pt-4 border-bottom-0">
            <h5 class="mb-0 fs-18 fw-700 text-dark">{{ translate('Invite System') }}</h5>
        </div>
        <div class="card-body py-0">
            <div class="row">
                <div class="col-md-12 fs-14 fs-14 ">
                    <label class="my-0 bg-success text-white rounded-2 p-2">
                        {{ single_price(env('INVITE_AMOUNT')) }}
                        {{ translate('Bonus for each invited user.') }} {{ translate('And each invited user will get') }}
                        {{ single_price(env('INVITE_DISCOUNT')) }}
                        {{ translate('discount for first order .') }}
                    <br>
                        {{ translate('When the invited user makes an order, the bonus will be added to your account.') }}
                    </label>
                </div>
        ...
        <div class="card-body">
            <div class="row">
                <label class="col-md-4 fs-14 fs-14">{{ translate('Invitations Number') }} :</label>
                <label class="col-md-8 fs-14 fs-14">{{ Auth::user()->invitedUsers->count() }}
                    {{ translate('Invitation') }}</label>
            </div>

            <div class="row">
                <label class="col-md-4 fs-14 fs-14">{{ translate('Invitations Amount') }} :</label>
                <label class="col-md-8 fs-14 fs-14">{{ single_price(Auth::user()->invitedUsers->count() * env('INVITE_AMOUNT')) }}</label>
            </div>

            <div class="row">
                <label class="col-md-4 fs-14 fs-14">{{ translate('Pending Invitations Number') }} :</label>
                <label class="col-md-8 fs-14 fs-14">{{ Auth::user()->pendingInvitedUsers->count() }}
                    {{ translate('Invitation') }}</label>
            </div>

            <div class="row">
                <label class="col-md-4 fs-14 fs-14">{{ translate('Pending Invitations Amount') }} :</label>
                <label class="col-md-8 fs-14 fs-14">{{ single_price(Auth::user()->pendingInvitedUsers->count() * env('INVITE_AMOUNT')) }}</label>
            </div>

            <div class="row">
                <label class="col-md-4 fs-14 fs-14">{{ translate('Invitation Link') }} :</label>
                @if (get_setting('invite_system') == 1)
                    <label class="col-md-4 fs-13 fs-13"
                        dir="ltr">{{ route('user.registration') }}?invite_code={{ Auth::user()->invitation_code }}</label>
                    <div class="col-md-4"> <a href="javascript:void(0)" class="dropdown-item" onclick="copyUrl(this)"
                            data-url="{{ route('user.registration') }}?invite_code={{ Auth::user()->invitation_code }}">
            ...
            <div class="row">
                <label class="col-md-4 fs-14 fs-14">{{ translate('Invitation Code For Application') }} :</label>
                @if (get_setting('invite_system') == 1)
                    <label class="col-md-4 fs-14 fs-14"
                        dir="ltr">{{ Auth::user()->invitation_code }}</label>
                    <div class="col-md-4"> <a href="javascript:void(0)" class="dropdown-item" onclick="copyUrl(this)"
                            data-url="{{ Auth::user()->invitation_code }}">
                            <i class="las la-clipboard mr-2"></i>
                            <span>{{ translate('Copy Code') }}</span>
                        </a>
                    </div>
                @endif
            </div>
                ...
                <div class="">
                    <div class="border p-4 mb-4 position-relative">
                        <div class="row fs-14 mb-2 mb-md-0">
                            <span class="col-md-2 text-secondary">{{ translate('First Name') }}:</span>
                            <span class="col-md-8 text-dark">{{ $address->first_name }}</span>
                        </div>
                        <div class="row fs-14 mb-2 mb-md-0">
                            <span class="col-md-2 text-secondary">{{ translate('Last Name') }}:</span>
                            <span class="col-md-8 text-dark">{{ $address->last_name }}</span>
                        </div>
                        <div class="row fs-14 mb-2 mb-md-0">
                            <span class="col-md-2 text-secondary">{{ translate('ID Number') }}:</span>
                            <span class="col-md-8 text-dark">{{ $address->id_number }}</span>
                        </div>
                        <div class="row fs-14 mb-2 mb-md-0">
                        ...
                        @endif
                        <div class="dropdown position-absolute absolute-md-top-right pt-4 mr-1">
