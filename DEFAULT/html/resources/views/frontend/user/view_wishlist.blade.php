...
@section('panel_content')
    ...
    @if (count($wishlists) > 0)
        <div
            ...
            @foreach ($products as $whishlistId => $product)
                @if (isset($product['provider']))
                    <div class="aiz-card-box col py-3 text-center border-right border-bottom has-transition hov-shadow-out z-1"
                        id="wishlist_{{ $whishlistId }}">
                        <div class="position-relative h-140px h-md-200px img-fit overflow-hidden mb-3">
                            <!-- Image -->
                            <a href="@if (!empty($product)) {{ route('product.provider', ['provider' => $product['provider'], 'id' => $product['id']]) }} @endif"
                                class="d-block h-100">
                                <img src="@if (!empty($product)) {{ $product['thumbnail'] }} @endif"
                                    class="lazyload mx-auto img-fit"
                                    title="@if (!empty($product)) {{ $product['name'] }} @endif"
                                    onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder.webp') }}';">
                            </a>
                            <!-- Remove from wishlisht -->
                            <div class="absolute-top-right aiz-p-hov-icon">
                                <a href="javascript:void(0)" onclick="removeFromWishlist({{ $whishlistId }})"
                                    data-toggle="tooltip" data-title="{{ translate('Remove from wishlist') }}"
                                    data-placement="left">
                            ...
                            <a class="cart-btn absolute-bottom-left w-100 h-35px aiz-p-hov-icon text-white fs-13 fw-700 d-flex justify-content-center align-items-center"
                                href="javascript:void(0)" onclick="">{{ translate('Add to Cart') }}</a>
                        </div>
                        <!-- Product Name -->
                        ...
                        <div class="fs-14">
                            ...
                            @if (!empty($product))
                                @if ($product['new_price'] != $product['unit_price'])
                                    <del class="opacity-60 ml-1">{{ single_price($product['unit_price']) }}</del>
                                @endif
                            @endif
                        </div>
                    </div>
                @elseif(isset($product['trendyol']))
                    <div class="aiz-card-box col py-3 text-center border-right border-bottom has-transition hov-shadow-out z-1"
                        ...
                        <div class="position-relative h-140px h-md-200px img-fit overflow-hidden mb-3">
                            <!-- Image -->
                            <a href="@if (!empty($product)) {{ route('trendyol-product', ['id' => $product['id'], 'urunNo' => $product['urunNo']]) }} @endif"
                                class="d-block h-100">
                                <img src="@if (!empty($product)) {{ $product['photos'][0] }} @endif"
                                    class="lazyload mx-auto img-fit"
                                    title="@if (!empty($product)) {{ $product['name'] }} @endif"
                                    onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder.webp') }}';">
                            </a>
                            ...
                            <a class="cart-btn absolute-bottom-left w-100 h-35px aiz-p-hov-icon text-white fs-13 fw-700 d-flex justify-content-center align-items-center"
                                ...
                                onclick="@if (!empty($product)) showAddToCartModal({{ $product['id'] }},1,{{ $product['urunNo'] }}) @endif">{{ translate('Add to Cart') }}</a>
                        </div>
                        <!-- Product Name -->
                        <h5 class="fs-14 mb-0 lh-1-5 fw-400 text-truncate-2 mb-3">
                            <a href="@if (!empty($product)) {{ route('trendyol-product', ['id' => $product['id'], 'urunNo' => $product['urunNo']]) }} @endif"
                    ...
                    <div class="aiz-card-box col py-3 text-center border-right border-bottom has-transition hov-shadow-out z-1"
                        ...
                        <div class="position-relative h-140px h-md-200px img-fit overflow-hidden mb-3">
                            ...
                            <a href="{{ route('product', $product->slug) }}" class="d-block h-100">
                                <img src="{{ uploaded_asset($product->thumbnail_img) }}" class="lazyload mx-auto img-fit"
                            ...
                            <a class="cart-btn absolute-bottom-left w-100 h-35px aiz-p-hov-icon text-white fs-13 fw-700 d-flex justify-content-center align-items-center"
                                href="javascript:void(0)"
                                onclick="showAddToCartModal({{ $product->id }})">{{ translate('Add to Cart') }}</a>
                        </div>
                        <!-- Product Name -->
                        <h5 class="fs-14 mb-0 lh-1-5 fw-400 text-truncate-2 mb-3">
                        ...
                        <!-- Price -->
                        <div class="fs-14">
                            <span class="fw-600 text-primary">{{ home_discounted_base_price($product) }}</span>
                            @if (home_base_price($product) != home_discounted_base_price($product))
                                <del class="opacity-60 ml-1">{{ home_base_price($product) }}</del>
                            @endif
                        </div>
                    </div>
                @endif
