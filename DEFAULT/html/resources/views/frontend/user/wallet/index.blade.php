    ...
    <div class="row gutters-16 mb-2">
        ...
        <div class="col-md-4 mx-auto mb-4">
            <div class="p-4 mb-3 c-pointer text-center bg-light has-transition border h-100 hov-bg-soft-light"
                onclick="wallet_SyCard_modal()">
                <span
                    class="size-60px rounded-circle mx-auto bg-dark d-flex align-items-center justify-content-center mb-3">
                    <i class="las la-plus la-3x text-white"></i>
                </span>
                <div class="fs-14 fw-600 text-dark">{{ translate('Recharge Wallet By SYCard') }}</div>
            </div>
        </div>

        <div class="col-md-4 mx-auto mb-4">
            <div class="p-4 mb-3 c-pointer text-center bg-light has-transition border h-100 hov-bg-soft-light"
                onclick="transfer_money()">
                <span
                    class="size-60px rounded-circle mx-auto bg-dark d-flex align-items-center justify-content-center mb-3">
                    <i class="las la-exchange-alt la-2x text-white"></i>
                </span>
                <div class="fs-14 fw-600 text-dark">{{ translate('Transfer Money') }}</div>
            </div>
        </div>
                ...
                <tbody class="fs-14">
                    @foreach ($wallets as $key => $wallet)
                        <tr>
                            ...
                            <td>@if($wallet->payment_method == 'SY Card' || $wallet->payment_method == 'inner transfer' || $wallet->payment_method == 'invite')
                                {{ ucfirst($wallet->payment_details) }}
                                @endif
                                </td>
...
@section('modal')
     ...
     <!-- Wallet SYCard Recharge Modal -->
    @include('frontend.'.get_setting('homepage_select').'.partials.wallet_SyCard_modal')

    <!-- Transfer Money Modal-->
    @include('frontend.'.get_setting('homepage_select').'.partials.transfer_money')
...
@section('script')
    <script type="text/javascript">
        ...
        function wallet_SyCard_modal() {
            $('#wallet_SyCard_modal').modal('show');
        }
