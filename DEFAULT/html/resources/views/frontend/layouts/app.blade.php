...
<body>
    ...
    <script>
        @if (Route::currentRouteName() == 'home' || Route::currentRouteName() == '/')
            ...
            $.post('{{ route('home.section.suggestions_products') }}', {
                _token: '{{ csrf_token() }}'
            }, function(data) {
                $('#suggestions_products').html(data);
                AIZ.plugins.slickCarousel();
            });
        ...
        $(document).ready(function() {
            $('.category-nav-element').each(function(i, el) {
                $(el).hover(
                    function() {
                        var $subCatMenu = $(this).find('.sub-cat-menu');
                        $subCatMenu.data('timeout', setTimeout(function() {
                            if (!$subCatMenu.hasClass('loaded')) {
                                $.post('{{ route('category.elements') }}', {
                                    _token: AIZ.data.csrf,
                                    id: $(el).data('id')
                                    }, function(data) {
                                        setTimeout(()=>  $subCatMenu.addClass('loaded').html(data), 200);
                                    if(data != 0){
                                        $subCatMenu.css({ 'z-index': 9, 'opacity': 1, 'transition': 'opacity 0.3s ease-in' , 'display': 'block'});
                                        $subCatMenu.addClass('hasData');
                                    }
                                });
                            }
                            else if($subCatMenu.hasClass('hasData')){
                            $subCatMenu.css({ 'z-index': 9, 'opacity': 1, 'transition': 'opacity 0.3s ease-in' , 'display': 'block'});
                            }
                        }, 500));
                    },
                    ...
                    function() {
                        var $subCatMenu = $(this).find('.sub-cat-menu');
                        clearTimeout($subCatMenu.data('timeout'));
        ...
        function addToCompare(id,product_name,trendyol,urunNo = null ){
            $.post('{{ route('compare.addToCompare') }}', {_token: AIZ.data.csrf, id:id, product_name:product_name , trendyol:trendyol , urunNo:urunNo }, function(data){
                if(data != 0){
                        $('#compare').html(data);
                        AIZ.plugins.notify('success', "{{ translate('Item has been added to compare list') }}");
                    }
                    else{
                        AIZ.plugins.notify('warning', "{{ translate('You cannot add more than three items or the same item twice') }}");
                    }
        ...
        function addToCompareProvider(id,provider){
            $.post('{{ route('compare.addToCompare') }}', {_token: AIZ.data.csrf, id:id, provider:provider}, function(data){
                if(data != 0){
                        $('#compare').html(data);
                        AIZ.plugins.notify('success', "{{ translate('Item has been added to compare list') }}");
                    }
        ...
        function addToWishList(id,urunNo,trendyol){
            @if (Auth::check() && Auth::user()->user_type == 'customer')
                $.post('{{ route('wishlists.store') }}', {_token: AIZ.data.csrf, id:id , urunNo:urunNo , trendyol:trendyol}, function(data){
                    if(data != 0){
                        $('#wishlist').html(data);
        ...
        function addToWishListProvider(id, provider){
        @if (Auth::check() && Auth::user()->user_type == 'customer')
            $.post('{{ route('wishlists.store') }}', {_token: AIZ.data.csrf, id:id, provider:provider}, function(data){
                if(data != 0){
                    $('#wishlist').html(data);
                    AIZ.plugins.notify('success', "{{ translate('Item has been added to wishlist') }}");
                }
        ...
        function showAddToCartModal(id,trendyol,urunNo = null){
            ...
            $('#addToCart').modal();
            $('.c-preloader').show();
            $.post('{{ route('cart.showCartModal') }}', {_token: AIZ.data.csrf, id: id, trendyol: trendyol, urunNo: urunNo}, function(data){
                $('.c-preloader').hide();
        ...
        function showAddProviderToCartModal(id,provider){
            if(!$('#modal-size').hasClass('modal-lg')){
                $('#modal-size').addClass('modal-lg');
            }
            $('#addToCart-modal-body').html(null);
            $('#addToCart').modal();
            $('.c-preloader').show();
            $.post('{{ route('cart.showCartModal') }}', {_token: AIZ.data.csrf, id: id, provider: provider}, function(data){
                $('.c-preloader').hide();
                $('#addToCart-modal-body').html(data);
                AIZ.plugins.slickCarousel();
                AIZ.plugins.zoom();
        ...
        function getVariantPrice() {
            ...
            if ($('#option-choice-form input[name=quantity]').val() > 0 && ($('#option-choice-form input[name=trendyol]').val() == 0 || $('#option-choice-form input[name=trendyol]').length == 0)) {
                $.ajax({
                    type: "POST",
                    ...
                    error: function(xhr, status, error) {
                        // Debugging: Log any errors from the AJAX request
        ...
        function checkAddToCartValidity(){
            var names = {};
            $('#option-choice-form input:radio:not(.not)').each(function() { // find unique names
            ...
            if($('#option-choice-form input:radio:not(.not):checked').length == count){
                return true;
            }
        ...
        function deleteSearchResult(id){
            @if (Auth::check())
             $.ajax({
                    type:"POST",
                    url: '{{ route('search.result.delete') }}',
                    data: {'id': id , _token: AIZ.data.csrf},
                    success: function(data){
                        if(data.success){
                           $('#search').focus();
                        }
