...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="row gutters-5">
                <div class="col text-md-left text-center">
                    @if (json_decode($order->shipping_address))
                        <address>
                            ...
                            {{ json_decode($order->shipping_address)->id_number ?? '' }}<br>
                ...
                <div class="col-sm-4 col text-md-left text-center">
                    @if (count($AlltrendyolDetails) > 0)
                        @foreach ($AlltrendyolDetails as $trendyolDetails)
                            <table>
                                <tbody>
                                    <tr>
                                        <td class="text-main text-bold w-50">{{ translate('Trendyol Order Details') }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-main text-bold">{{ translate('Order #') }}</td>
                                        <td class="text-info text-bold text-right">
                                            {{ $trendyolDetails['summary']['orderNumber'] }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-main text-bold">{{ translate('Address') }}</td>
                                        <td class="text-right">{{ $trendyolDetails['summary']['address'] }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-main text-bold">{{ translate('State') }}</td>
                                        <td class="text-right">{{ $trendyolDetails['summary']['state'] }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-main text-bold">{{ translate('City') }}</td>
                                        <td class="text-right">{{ $trendyolDetails['summary']['city'] }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-main text-bold">{{ translate('Street Address') }}</td>
                                        <td class="text-right">{{ $trendyolDetails['summary']['streetAddress'] }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-main text-bold">{{ translate('Address Type') }}</td>
                                        <td class="text-right">{{ $trendyolDetails['summary']['addressType'] }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        @endforeach
                    ...
                    <table class="table-bordered aiz-table invoice-summary table">
                        <thead>
                            <tr class="bg-trans-dark">
                                ...
                                <th data-breakpoints="lg" class="text-uppercase">{{ translate('Delivery Status') }}
                                </th>
                                <th data-breakpoints="lg" class="text-uppercase">{{ translate('Delivery Type') }}</th>
                                <th data-breakpoints="lg" class="text-uppercase">{{ translate('Tracking') }}</th>
                                <th data-breakpoints="xl" class="min-col text-uppercase text-center">
                                    {{ translate('Qty') }}
                                </th>
                                <th data-breakpoints="xl" class="min-col text-uppercase text-center">
                                    {{ translate('Price') }}</th>
                                <th data-breakpoints="xl" class="min-col text-uppercase text-right">
                                    {{ translate('Total') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @elseif($orderDetail->trendyol == 1)
                                            <a href="{{ route('trendyol-product', ['id' => $orderDetail['product_id'], 'urunNo' => $orderDetail['urunNo']]) }}"
                                                target="_blank">
                                                <img height="50" src="{{ $orderDetail->product_image }}">
                                            </a>
                                    ...
                                    <td>
                                        @if (auth()->user()->can('update_order_delivery_status') &&
                                            ...
                                            <select class="form-control py-1 px-3 update_order_detail_delivery_status"
                                                style="cursor: pointer;" data-id="{{ $orderDetail->id }}">
                                                <option value="pending" @if ($orderDetail->delivery_status == 'pending') selected @endif>
                                                    {{ translate('Pending') }}
                                                </option>
                                                <option value="confirmed"
                                                    @if ($orderDetail->delivery_status == 'confirmed') selected @endif>
                                                    {{ translate('Confirmed') }}
                                                </option>
                                                <option value="picked_up"
                                                    @if ($orderDetail->delivery_status == 'picked_up') selected @endif>
                                                    {{ translate('Picked Up') }}
                                                </option>
                                        ...
                                        @else
                                            <input type="text" class="form-control"
                                                value="{{ $orderDetail->delivery_status }}" disabled>
                                        @endif
                                    </td>
                                    <td>
                                    ...
                                    @php
                                        array_push($trendyolProducts, $orderDetail['product_id']);
                                    @endphp
                                </tr>
                            @endforeach
                        </tbody>
    ...
    @if (count($AlltrendyolDetails) > 0)
        @foreach ($AlltrendyolDetails as $trendyolDetails)
            <div class="card">
                <div class="card-header">
                    <h1 class="h2 fs-16 mb-0">{{ translate('Trendyol Shipment Status') }}</h1>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-lg-12 table-responsive">
                            <table class="table-bordered aiz-table invoice-summary table">
                                <thead>
                                    <tr class="bg-trans-dark">
                                        ...
                                        <th data-breakpoints="lg" class="text-uppercase">
                                            {{ translate('Shipping Tracking No.') }}
                                        </th>
                                        <th data-breakpoints="lg" class="text-uppercase">
                                            {{ translate('Delivery altStatus') }}</th>
                                        <th data-breakpoints="lg" class="min-col text-uppercase text-center">
                                            {{ translate('Qty') }}
                                        </th>
                                        <th data-breakpoints="lg" class="min-col text-uppercase text-right">
                                            {{ translate('Estimated Delivery Date') }}</th>
                                        <th data-breakpoints="lg" class="min-col text-uppercase text-right">
                                            {{ translate('Delivery Date') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @php
                                        $index = 0;
                                    @endphp
                                    @foreach ($trendyolDetails['products'] as $orderDetail)
                                        @if (in_array($orderDetail['productId'], $trendyolProducts))
                                            <tr>
                                                <td>{{ ++$index }}</td>
                                                <td><a target="_blank" href="{{ $orderDetail['link'] }} "><img
                                                            height="50" src="{{ $orderDetail['image'] }}"></a></td>
                                                <td><a target="_blank" href="{{ $orderDetail['link'] }}"
                                                        class="text-muted">{{ $orderDetail['name'] }}</a></td>
                                                <td class="text-center">
                                                    {{ $orderDetail['status'] }}
                                                </td>
                                                <td class="text-center">
                                                    {{ $orderDetail['kargoTakipNo'] }}
                                                </td>
                                                <td class="text-center">
                                                    {{ $orderDetail['altStatus'] }}
                                                </td>
                                                <td class="text-center">
                                                    {{ $orderDetail['count'] }}
                                                </td>
                                                <td class="text-center" dir="ltr">
                                                    {{ $orderDetail['minDate'] }} - {{ $orderDetail['maxDate'] }}
                                                </td>
                                                <td class="text-center" dir="ltr">
            ...
            <div class="modal-content p-2rem">
                <div class="modal-body text-center">
                    ...
                    <p class="mt-3 mb-3 fs-16 fw-700">
                        {{ translate('Are you sure you want to change the payment status?') }}</p>
...
@section('script')
    <script type="text/javascript">
        $(document).on('change', '.update_order_detail_delivery_status', function() {
            var order_detail_id = $(this).data("id");
            var status = $(this).val();
            $.post('{{ route('order_detail.update_delivery_status') }}', {
                _token: '{{ @csrf_token() }}',
