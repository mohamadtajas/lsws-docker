    ...
    @can('admin_dashboard')
        ...
        <div class="row gutters-10">
            <div class="col-md-6">
                <div class="card">
                    <div class="card-header">
                        <h6 class="mb-0 fs-14">{{ translate('Number of Customers') }} : {{$number_of_customers_statistics}}</h6>
                        <div class="form-inline">
                            <div class="form-group mb-0">
                                <input type="text" class="aiz-date-range form-control" id="customers_date"
                                    value="{{ \Carbon\Carbon::parse($customers_start_date)->format('d-m-Y') }} to {{ \Carbon\Carbon::parse($customers_end_date)->format('d-m-Y') }}"
                                    name="customer_date" placeholder="{{ translate('Filter by date') }}" data-format="DD-MM-Y"
                                    data-separator=" to " data-advanced-range="true" autocomplete="off" dir="ltr">
                            </div>
                            <button class="btn btn-primary ml-2" id="customers_filter_btn">{{ translate('Filter') }}</button>
                        </div>
                    </div>
                    <div class="card-body">
            ...
            <div class="col-md-6">
                <div class="card">
                    <div class="card-header">
                        <h6 class="mb-0 fs-14">{{ translate('Number of Orders') }} : {{$number_of_orders_statistics}}</h6>
                        <div class="form-inline">
                            <div class="form-group mb-0">
                                <input type="text" class="aiz-date-range form-control" id="orders_date"
                                    value="{{ \Carbon\Carbon::parse($orders_start_date)->format('d-m-Y') }} to {{ \Carbon\Carbon::parse($orders_end_date)->format('d-m-Y') }}"
                                    name="orders_date" placeholder="{{ translate('Filter by date') }}" data-format="DD-MM-Y"
                                    data-separator=" to " data-advanced-range="true" autocomplete="off" dir="ltr">
                            </div>
                            <button class="btn btn-primary ml-2" id="orders_filter_btn">{{ translate('Filter') }}</button>
                        </div>
                    </div>
                    <div class="card-body">
...
@section('script')
    <script type="text/javascript">
        ...
        document.addEventListener("DOMContentLoaded", function () {
            // JavaScript for date range filter button
            document.getElementById('customers_filter_btn').addEventListener('click', function () {
                const dateRange = document.querySelector('#customers_date').value;

                // Split the date range by the " to " separator to get start and end dates
                const dates = dateRange.split(' to ');
                const startDate = dates[0].trim();
                const endDate = dates[1] ? dates[1].trim() : '';

                // Redirect to the same page with new query parameters if dates are valid
                if (startDate && endDate) {
                    window.location.href = `{{ route('admin.dashboard') }}?customers_start_date=${startDate}&customers_end_date=${endDate}`;
                } else {
        ...
        // Initialize the chart for graph-3 after a new date range is selected
        AIZ.plugins.chart('#graph-3', {
            type: 'bar',
            data: {
                labels: [
                    @foreach (explode(',', $number_of_customers) as $index => $customer)
                        '{{ \Carbon\Carbon::parse($customers_start_date)->addDays($index)->format('d-m-Y') }}',
                    @endforeach
                ],
                datasets: [{
                    ...
                    backgroundColor: [
                        @foreach (explode(',', $number_of_customers) as $customer)
                            'rgba(145,131,208,0.4)',
                        @endforeach
                    ],
                    borderColor: [
                        @foreach (explode(',', $number_of_customers) as $customer)
        ...
        document.addEventListener("DOMContentLoaded", function () {
            // JavaScript for date range filter button
            document.getElementById('orders_filter_btn').addEventListener('click', function () {
                const dateRange = document.querySelector('#orders_date').value;

                // Split the date range by the " to " separator to get start and end dates
                const dates = dateRange.split(' to ');
                const ordersStartDate = dates[0].trim();
                const ordersEndDate = dates[1] ? dates[1].trim() : '';

                // Redirect to the same page with new query parameters if dates are valid
                if (ordersStartDate && ordersEndDate) {
                    window.location.href = `{{ route('admin.dashboard') }}?orders_start_date=${ordersStartDate}&orders_end_date=${ordersEndDate}`;
                } else {
                    alert("Please select a valid date range.");
                }
            });
        ...
        AIZ.plugins.chart('#graph-4', {
            ...
            data: {
                labels: [
                    @foreach (explode(',', $number_of_orders) as $index => $customer)
                        '{{ \Carbon\Carbon::parse($orders_start_date)->addDays($index)->format('d-m-Y') }}',
                    @endforeach
                ],
                datasets: [{
                    ...
                    backgroundColor: [
                        @foreach (explode(',', $number_of_orders) as $customer)
                            'rgba(102,145,217,0.4)',
                        @endforeach
                    ],
                    borderColor: [
                        @foreach (explode(',', $number_of_orders) as $customer)
                            'rgba(102,145,217,0.8)',
                        @endforeach
                    ],
                    borderWidth: 1
                }]
