                    ...
                    <div class="col-md-3 ml-auto">
                        <select class="form-control form-control-sm aiz-selectpicker mb-2 mb-md-0" id="user_id"
                            ...
                            @foreach (App\Models\User::where('user_type', '=', 'admin')->orWhere('user_type', '=', 'seller')->get() as $key => $seller)
                                <option value="{{ $seller->id }}" @if ($seller->id == $seller_id) selected @endif>
                                    {{ $seller->shop->name ?? '' }} ({{ $seller->name }})</option>
                            @endforeach
                        </select>
                    </div>
                @endif
                @if (get_setting('product_approve_by_admin') == 1 && ($type == 'Seller' || $type == 'All'))
                <div class="col-md-2 ml-auto">
                    <select class="form-control form-control-sm aiz-selectpicker mb-2 mb-md-0" id="user_id"
                        name="approved_filter" onchange="sort_products()">
                        <option value="">{{ translate('Approved / Not Approved') }}</option>
                        <option value="approved" @if ($approved_filter == 'approved') selected @endif>{{ translate('Approved') }}</option>
                        <option value="not_approved" @if ($approved_filter == 'not_approved') selected @endif>{{ translate('Not Approved') }}</option>
                    </select>
                </div>
                @endif
                <div class="col-md-2 ml-auto">
