        ...
        <form class="form-horizontal" action="{{ route('languages.key_value_store') }}" method="POST">
            ...
            <div class="card-body">
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="text" class="form-control value" style="width:100%"
                                        ...
                                        @php
                                    $translations = new \App\Models\Translation();
                                    $translate_lang = $translations->where('lang', 'Like',  $language->code)
                                    ->where('lang_key', 'Like', $translation->lang_key)
                                    ->first(); @endphp
                                        @if ($translate_lang) value="{{ $translate_lang->lang_value }}" @endif>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                ...
                <div class="aiz-pagination">
                    {{ $lang_keys->withPath(url()->current())->appends(request()->input())->links() }}
                </div>
