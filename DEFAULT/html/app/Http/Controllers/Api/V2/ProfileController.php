...
use App\Http\Requests\Profile\UpdateDeviceTokenRequest;
...
class ProfileController extends Controller
{
    ...
    public function update_device_token(UpdateDeviceTokenRequest $request)
    {
        $user = User::find(auth()->user()->id);
        if (!$user) {
            return response()->json([
    ...
    public function imageUpload(ImageRequest $request)
    {
        ...
        try {
            ...
            if (env('FILESYSTEM_DRIVER') != 'local') {
                Storage::disk(env('FILESYSTEM_DRIVER'))->put($newPath, file_get_contents(base_path('public/') . $newPath));
                unlink(base_path('public/') . $newPath);
            }
    ...
    public function generate_invite_code()
    {
        ...
        return response()->json([
            'result' => true,
            'message' => translate("Invitation url generated"),
            'invitation_code' => get_setting('invite_system') == 1 ? $user->invitation_code : "",
            'invite_amount' => single_price(env('INVITE_AMOUNT')),
            'invite_discount' => single_price(env('INVITE_DISCOUNT')),
            'invitation_number' => $user->invitedUsers->count(),
            'invitation_amount' => single_price($user->invitedUsers->count() * env('INVITE_AMOUNT')),
            'pending_invitations_number' => $user->pendingInvitedUsers->count(),
            'pending_invitations_amount' => single_price($user->pendingInvitedUsers->count() * env('INVITE_AMOUNT')),
        ]);
    }
}
