...
class AuthController extends Controller
{
    public function signup(Request $request)
    {
        ...
        $invited_before = Invitation::where('invited_user', $user->email)->first();
        if (get_setting('invite_system') == 1 && !$invited_before) {
            $isNew = true;
        } else {
            $isNew = false;
        }
        return $this->loginSuccess($user, null, $isNew);
    }
    ...
    public function socialLogin(Request $request)
    {
        ...
        if ($existingUserByProviderId) {
            ...
            $existing_user = User::where('email', $social_user_details->email)->first();
            $invited_before = Invitation::where('invited_user',  $social_user_details->email)->first();
            if (!$existing_user && get_setting('invite_system') == 1 && !$invited_before) {
                $isNew = true;
            }
            ...
            return $this->loginSuccess($existing_or_new_user, null, $isNew);
        }
    }

    public function loginSuccess($user, $token = null, $isNew = false)
    {
    ...
    public function account_deletion()
    {
        if (auth()->user()->balance != 0) {
            return response()->json([
                "result" => false,
                "message" => translate('You can not delete your account , your balance must be zero .')
            ]);
        }
    ...
    {
        $user = auth()->user();
        if (get_setting('invite_system') == 1 && isset($request->invite_code) && $request->invite_code != null && $user != null) {
            $invited_by_user = User::where('invitation_code', $request->invite_code)->first();
            if ($invited_by_user) {
                $invited = Invitation::where('invited_user', $user->email)->first();
                if ($invited) {
                    return response()->json([
                        'result' => false,
                        'message' => translate('Already Invited')
                    ]);
                }
                $invited = new Invitation;
                $invited->invited_user = $user->email;
                $invited->invited_by_user = $invited_by_user->email;
                $invited->save();
                return response()->json([
                    'result' => true,
                    'message' => translate('Invited successfully')
                ]);
            } else {
                return response()->json([
                    'result' => false,
                    'message' => translate('Invited user not found')
                ]);
            }
        } else {
            return response()->json([
