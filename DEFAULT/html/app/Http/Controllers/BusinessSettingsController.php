...
use App\Http\Requests\BusinessSettings\UpdateActivationRequest;
use App\Http\Requests\BusinessSettings\UpdateEnvKeyRequest;
use App\Http\Requests\BusinessSettings\UpdateFaceBookChatRequest;
use App\Http\Requests\BusinessSettings\UpdateFaceBookCommentRequest;
use App\Http\Requests\BusinessSettings\UpdateFaceBookPixelRequest;
use App\Http\Requests\BusinessSettings\UpdateGoogleAnalyticsRequest;
use App\Http\Requests\BusinessSettings\UpdateGoogleFirebaseRequest;
use App\Http\Requests\BusinessSettings\UpdateGoogleMapRequest;
use App\Http\Requests\BusinessSettings\UpdateGoogleRecaptchaRequest;
use App\Http\Requests\BusinessSettings\UpdatePaymentMethodRequest;
use App\Http\Requests\BusinessSettings\UpdateRequest;
use App\Http\Requests\BusinessSettings\UpdateSellerVerificationFormRequest;
use App\Http\Requests\BusinessSettings\UpdateShippingConfigurationRequest;
...
class BusinessSettingsController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware(['permission:external_providers'])->only('external_providers');
    }
    ...
    public function google_firebase()
    {
        // CoreComponentRepository::instantiateShopRepository();
        // CoreComponentRepository::initializeCache();
     ...
     */
    public function payment_method_update(UpdatePaymentMethodRequest $request)
    {
     ...
     * @return \Illuminate\Http\Response
     */
    public function google_analytics_update(UpdateGoogleAnalyticsRequest $request)
    {
        foreach ($request->types as $key => $type) {
            $this->overWriteEnvFile($type, $request[$type]);
        }
    ...
    public function google_recaptcha_update(UpdateGoogleRecaptchaRequest $request)
    {
        foreach ($request->types as $key => $type) {
            $this->overWriteEnvFile($type, $request[$type]);
        }
    ...
    public function google_map_update(UpdateGoogleMapRequest $request)
    {
        foreach ($request->types as $key => $type) {
            $this->overWriteEnvFile($type, $request[$type]);
        }
    ...
    public function google_firebase_update(UpdateGoogleFirebaseRequest $request)
    {
        foreach ($request->types as $key => $type) {
            $this->overWriteEnvFile($type, $request[$type]);
        }
     ...
     * @return \Illuminate\Http\Response
     */
    public function facebook_chat_update(UpdateFaceBookChatRequest $request)
    {
        foreach ($request->types as $key => $type) {
            $this->overWriteEnvFile($type, $request[$type]);
        }
    ...
    public function facebook_comment_update(UpdateFaceBookCommentRequest $request)
    {
        foreach ($request->types as $key => $type) {
            $this->overWriteEnvFile($type, $request[$type]);
        }
    ...
    public function facebook_pixel_update(UpdateFaceBookPixelRequest $request)
    {
        foreach ($request->types as $key => $type) {
            $this->overWriteEnvFile($type, $request[$type]);
        }
     ...
     * @return \Illuminate\Http\Response
     */
    public function env_key_update(UpdateEnvKeyRequest $request)
    {
        foreach ($request->types as $key => $type) {
            $this->overWriteEnvFile($type, $request[$type]);
        }
     ...
     * @return \Illuminate\Http\Response
     */
    public function seller_verification_form_update(UpdateSellerVerificationFormRequest $request)
    {
        $form = array();
    ...
    public function updateActivationSettings(UpdateActivationRequest $request)
    {
    ...
    public function vendor_commission_update(UpdateRequest $request)
    {
        foreach ($request->types as $key => $type) {
            $business_settings = BusinessSetting::where('type', $type)->first();
            if ($business_settings != null) {
    ...
    public function shipping_configuration_update(UpdateShippingConfigurationRequest $request)
    {
        $business_settings = BusinessSetting::where('type', $request->type)->first();
    ...
    {
        return view('backend.setup_configurations.app_settings');
    }

    public function external_providers()
    {
        $likeCard = new LikeCardService();
        $providers = [
            [
                'name' => 'Trendyol',
                'status' => trendyol_login(env('TRENDYOL_SEARCH_USERNAME') , env('TRENDYOL_SEARCH_PASSWORD')) != null ? 'active' : 'disable',
                'balance' => 0
            ],
            [
                'name' => 'Like Card',
                'status' => $likeCard->checkBalance() ? 'active' : 'disable',
                'balance' => $likeCard->checkBalance() ? $likeCard->getBalance() : 0
            ],
            [
                'name' => 'Chat Gpt',
                'status' => test_chat_gpt_connection() ? 'active' :'disable',
                'balance' => 0
            ],
        ];

        return view('backend.setup_configurations.external_providers', compact('providers'));
    }
}
