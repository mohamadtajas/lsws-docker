...
class CartUtility
{
    ...
    public static function save_cart_data($cart, $product, $price, $tax, $quantity, $trendyol_price = 0)
    {
        $cart->quantity = $quantity;
        $cart->product_id = $product['id'];
        $cart->owner_id = $product['user_id'] ?? get_admin()->id;
        $cart->price = $price;
        $cart->tax = $tax;
        $cart->product_referral_code = null;
        $cart->trendyol_price = $trendyol_price;
    ...
    public static function check_auction_in_cart($carts)
    {
        foreach ($carts as $cart) {
            if ($cart->trendyol == 0 && $cart->provider_id == null) {
